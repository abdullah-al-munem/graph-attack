...
from utils import get_dataset_from_deeprobust, destructuring_dataset, get_target_node_list
...
def test_acc_GCN(adj, features, data,target_node):
    ...
    return acc_test.item(), output.argmax(1)[target_node], labels[target_node], probs.cpu().detach().numpy()
...
class ProposedAttack:
    ...
    def attack(self, target_node, n_perturbations, isBoth=1, isAdd=0, isRemove=0):
        ...
        while n_perturbations:
            ...
            attack_history.append(1 - modified_adj[tuple(best_edge)])
            adj_norm = normalize_adj(modified_adj)
        ...
        return modified_adj, attack_history
    
    def predict(self, modified_adj, target_node):
        return test_acc_GCN(modified_adj, self.data2.features, self.data2, target_node)
...
def start_attack_proposed_model(surrogate_model, dataset, defense_model, budget_range, node_list, times=1):
    ...
    for budget in tqdm.tqdm(range(start_budget, budget_range+1)):
        ...
        for target_node in tqdm.tqdm(node_list):
            ...
            modified_adj, attack_history = proposed_model.attack(target_node=target_node, n_perturbations=budget)
            # print("haha!..")
            end_time = time.time()
            
            accuracy, pred, actual, probs = proposed_model.predict(modified_adj, target_node)
            print("accuracy = ", accuracy)

            return attack_history, accuracy, pred, actual
